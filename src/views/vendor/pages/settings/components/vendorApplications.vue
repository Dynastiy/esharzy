      ...
      <div>
        <div>
            <div class="text-right">
                <button :class="{inactive: inactive.length > 0 }">Become a Vendor</button>
            </div>
          <!-- List -->
          <section class="">
...
export default {
  ...
  methods: {
    ...
    goToKYC (item) {
      this.$router.push(`/vendor/applications/${item.id}`)
    },
    filter () {
      const payload = {
        q: 'status',
  ...
  computed: {
    ...mapState('user', ['kycs']),
    inactive () {
      const inactive = this.kycs.filter(item => item.status === 'approved')
      return inactive
    }
  }
}
